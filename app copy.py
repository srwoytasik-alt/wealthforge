...
if page == "Dashboard":
    ...
    # )
        # Surplus with stronger color styling
    if surplus >= 0:
        color_style = "color: #28a745; font-weight: bold;"  # green
    else:
        color_style = "color: #dc3545; font-weight: bold;"  # red

    col1.markdown(
        f"""
        <div style="text-align: center;">
            <div style="font-size: 1rem; color: #6c757d;">Monthly Surplus</div>
            <div style="font-size: 2rem; {color_style}">${surplus:,.0f}</div>
        </div>
        """,
    ...
    if emergency_current > 0:
        ...
        # progress_value = min(emergency_current / EMERGENCY_GOAL, 1.0)
        # st.progress(progress_value)
        progress_value = min(emergency_current / EMERGENCY_GOAL, 1.0)

        st.progress(progress_value)

        if emergency_current >= EMERGENCY_GOAL:
            st.success("Emergency Fund Goal Reached 🎉")
        else:
             st.info(f"${EMERGENCY_GOAL - emergency_current:,.0f} remaining to goal")
